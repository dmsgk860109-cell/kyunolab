...
- Tag archive pages are generated by `npm run generate:tags`.
- The generator reads `data/stories.json` and `data/guides.json`.
- It supports both legacy `tag` fields and future `tags` arrays.
- Public URLs use `/tags/[tag-slug]/`.
- Do not edit generated files under `tags/` directly.
- The generator also refreshes tag URLs in `sitemap.xml`.
...
- Implemented as generated static archive pages.
- Needs future improvement only if related-tag clustering becomes useful.
