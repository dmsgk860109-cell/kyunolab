...
- 일반 텍스트가 아니라 Rich Text 서식을 적용하여 출력한다.
- 메인 제목은 반드시 Heading 1(H1) 서식을 사용한다.
- 소제목은 반드시 Heading 2(H2) 서식을 사용한다.
- 본문은 일반 문단(Paragraph) 서식을 사용한다.
- 제목은 굵게(Bold)만 표시하지 않는다.
- 소제목도 굵게만 표시하지 않는다.
- 제목과 소제목은 본문보다 실제 글자 크기가 크게 보여야 한다.
- 문단 사이에는 완전히 빈 줄을 사용하지 않는다.
- 문단 사이는 아래 문자 하나만 있는 줄을 사용한다.

ㅤ

- 위 문자를 삭제하거나 다른 문자로 변경하지 않는다.
- 출력 결과에서는 빈 줄처럼 보여야 한다.
- 네이버 블로그에 그대로 붙여넣었을 때 문단 간격이 유지되어야 한다.`;
  }
...
(() => {
  const article = document.querySelector('.archive-story');
  if (!article) return;
